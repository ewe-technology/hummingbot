...
class HyperliquidCandlesBalancePositions(ScriptStrategyBase):
    ...
    def on_tick(self):
        ...
        spot_connector_name = "hyperliquid"

        # 檢查 connector 是否存在
        perp_connector = self.connectors.get(perp_connector_name)
        spot_connector = self.connectors.get(spot_connector_name)
        ...
        self._log_market_price(perp_connector_name)
        self._log_market_price(spot_connector_name)
        ...
        if self._order_placed < self._target_building_times:
            ...
            self.logger().info("🎉 暫時進入idle模式 等之後建立判斷邏輯都成交後再關閉策略")
    ...
    # ========= 5. 市場價格 =========
    def _log_market_price(self, connector_name):
        perp_connector = self.connectors.get(connector_name)
        if perp_connector is None or not perp_connector.ready:
            return
        ...
        try:
            best_bid, best_ask = self._get_best_bid_ask(connector_name, trading_pair)
    ...
    # ========= 6. 限價期現建倉  =========
    def _place_demo_orders(self, spot_connector_name, perp_connector_name):
        """
        ...
        # 取得買一價賣一價當作參考價格
        spot_best_bid, spot_best_ask = self._get_best_bid_ask(spot_connector_name, trading_pair_spot)
        perp_best_bid, perp_best_ask = self._get_best_bid_ask(perp_connector_name, trading_pair_perp)

        if spot_best_bid is None:
            self.logger().warning("❌ 無法掛單：spot_best_bid 為 None")
            return
        ...
        self.logger().info("-" * 80)
        self.logger().info("🧾 建倉準備 -- 準備掛 BTC-USD 現價單")
        self.logger().info(f"  spot_best_bid = {spot_best_bid}")
        ...
        self.logger().info("-" * 80)
        self.logger().info("🧾 建倉準備 -- 準備掛 BTC-USD 現價單")
        self.logger().info(f"  best_bid = {perp_best_ask}")
    ...
    # ========= 7. 取得 best bid/ best ask price 函數
    def _get_best_bid_ask(self, connector_name: str, trading_pair: str):
        """
        回傳 (best_bid, best_ask)，若無資料則回傳 (None, None)
        """
        perp_connector = self.connectors.get(connector_name)
        if perp_connector is None or not perp_connector.ready:
            return None, None
        ...
        try:
            ...
            self.logger().debug(f"_get_best_bid_ask 取得訂單簿失敗: {e}")
