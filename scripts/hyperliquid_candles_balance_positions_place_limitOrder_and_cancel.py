...
class HyperliquidCandlesBalancePositions(ScriptStrategyBase):
    ...
    def _place_demo_orders(self):
        ...
        perp_connector = self.connectors.get("hyperliquid_perpetual")
        if perp_connector is None or not perp_connector.ready:
            self.logger().warning("❌ 無法掛單：connector 尚未就緒")
            return
        ...
        try:
            mid_price = perp_connector.get_mid_price(trading_pair)
        except Exception as e:
        ...
        # Buy / Sell 價格
        buy_price = mid_price * Decimal("0.95")   # 買在 mid 下 5%
        sell_price = mid_price * Decimal("1.05")  # 賣在 mid 上 5%

        self.logger().info("-" * 80)
        self.logger().info("🧾 [掛單示範] 準備掛 BTC-USD 限價單")
        self.logger().info(f"  mid_price = {mid_price}")
        self.logger().info(f"  buy  價格 = {buy_price}, 數量 = {order_amount}")
        self.logger().info(f"  sell 價格 = {sell_price}, 數量 = {order_amount}")
        ...
        try:
            ...
            self.logger().info(f"✅ BUY 掛單完成，order_id = {buy_order_id}")
        except Exception as e:
        ...
        try:
            sell_order_id = self.sell(
                ...
                order_type=OrderType.LIMIT,
                price=sell_price,
                position_action=PositionAction.OPEN,  # 這裡示範單純開倉
            )
            self.logger().info(f"✅ SELL 掛單完成，order_id = {sell_order_id}")
        except Exception as e:
